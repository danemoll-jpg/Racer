...
System.IO.File.WriteAllText("Docs/VEGETATION_TRUNK_ALIGNMENT.txt",$"{(missing==0&&extra==0?"PASS":"FAIL")} baked bark vertices versus authored collider corners, 1mm rounding: expected unique corners {expected.Count}, actual {actual.Count}; missing {missing}, extra {extra}; bark vertices {vertices}. Leaves have no colliders.\n");
