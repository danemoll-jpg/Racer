...
111/111 final desktop checks passed: 99 swept/record fixtures plus 12 actual dynamic-body FixedUpdate crossings. Every profile crossed with a backwards-facing body while moving forward, crossed high while spinning, and rejected genuine reverse travel. Other cases include shoulders, duplicates, teleports, finish rejection, reverse/re-traverse, sustained partial rejoin, shared AI credit and byte-preserving record migration. The dynamic tests set an initial pose/velocity and let normal physics/RaceDirector updates run; they do not manually invoke Sample during travel.

The final partial-rejoin correction also credits a physically crossed expected road gate while branch context is retained, even when that gate is outside the branch's designated bypass list. Holding recovery context must not suppress real road passage. The first review2 migration rerun reused an isolated folder containing new-rule records: its empty-new-record assertion failed because preservation correctly kept those files. That failure is retained in `rules-review2-reused-save/`; a fresh isolated run passed all111 checks without deleting any existing records.
...
Failures retained: the initial 10m aperture caught motorcycle/ATV lines. A 20m widening alone still failed motorcycle runs because redundant solid crossbeams overlapped the continuous ramp surface. Removing those12 embedded beam colliders preserved visible support and eliminated the repeated interior impacts. Final review2 motorcycle clean/varied/recovery runs:19.466/19.000/21.833s, both panes broken in each, zero charges/buzzes, minimum up .971/.970/.970. Road comparisons:19.333/19.366s. The stunt is fun-oriented: the cautious clean run is about .1s slower than the road, while the quicker varied run saves about .3s. Recovery costs time naturally.

The complete final ordinary-frame matrix passed80/80 attempts: four routes × four profiles × two branch lines, one local-recovery branch attempt and two road comparisons. There were48 verified branch exits,16 local recoveries, zero misses, zero penalty seconds and zero warning buzzes. All12 gully branch attempts broke both panes. These are normal FixedUpdate physics and virtual gamepad commands after initial scenario placement; they are not human driving. Reversal/abandonment exploit combinations additionally use the explicit swept fixtures, not a claim of exhaustive ordinary-frame coverage. See `routes-summary.json` and `../CR034-039/correction-release2-all/` for the complete traces, including lateral/airborne/up-vector measurements. Its77 invariant checks also passed.
...
Jamerson is raised pavement works: existing supported 40m/6.2m takeoff, asphalt surface, graded aggregate shoulder, orange breakable work markers and an open-right-lane sign/bypass. Collision retains the tested continuous takeoff. Eight final review2 ordinary-frame flights (two/profile) credited CP14 and landed. Motorcycle fast approach 51.224m/s, apex17.430m above ramp base, air3.263s. A full-speed ATV drifted about6.4m left at **later CP15**, outside its six-metre half-width: a genuine +5 miss, retained rather than counted as a clean run.
...
`audio-final/` contains actual listener DSP mix WAVs during physical authored-prop hits with engine throttle. All15 final review2 material/volume combinations broke the intended prop and emitted events. Default master.8 peaks .126–.218; lowered master.25 peaks .037–.064; all five master-zero captures have peak/RMS exactly0; no clipped samples. These are mixed audio samples, not just AudioSource assertions. OS-endpoint capture and subjective listening were **not** performed. Dan should judge impact prominence/material character on his actual speakers.

`audio-transients.csv` also measures100ms windows of the recorded PCM. Wood's default peak rises from .050 in the initial200ms to .175 around the impact; chain-link reaches .218 and glass .217. This is the complete engine/ambience/impact mix, not an isolated-source loudness comparison. Mailbox contact begins immediately, so its initial200ms is not a valid pre-impact baseline. Clips route directly through the listener; Master controls listener volume and Vehicle controls the impact voices.
...
Normal/Hard use more of each existing profile's cornering/braking capability, higher supported hill/crest targets, less Normal lifting and stronger throttle response. Easy's pace constants remain unchanged. [AI-RACES.md](AI-RACES.md) records exact full-race/lap/sector results, winner/reference gaps and traffic flow. Both current three-lap races finished all four racers without a miss or recovery. Best laps improved for all profiles versus the sparse-traffic baseline, but Hard tourer/motorcycle total races became slower in denser traffic; that limitation is explicitly retained. Automated reference-pilot gaps do not establish Dan's winning chance.

Normal/Hard traffic averaged11.49/11.68 cars on the highway and21.91/22.30 passes per minute, with transient occupancy2–17 as safe recycling deferred visible/near-player reuse. All four lanes were represented. Across294 recycles, the minimum player distance was220m. Traffic recoveries were zero; sampled core same-lane minimum centre gaps were26.45/24.52m. These measures do not prove junction contact or future congestion is impossible.

Fresh comparable performance runs used one foreground standalone at a time,1280×720, VSync off,120fps cap, Normal, original/tourer/motorcycle/ATV and one full lap. On the i7-9700 / GTX1660Ti, old0.6.0 with4 traffic measured8.33ms median /8.35ms p95 and482.2MiB peak working set; review2 with20 traffic measured8.33/8.34ms and480.6MiB. Peak memory was sampled externally from the Windows process because Unity's embedded process counter returned0. This capped single-run comparison shows no observed regression under these conditions; it is not proof of lower true rendering cost or worst-case performance. Exact results/settings/source hashes: `performance-comparison.json`, `performance-before/`, `performance-after/`.

## Evidence and remaining review

The final visible standalone flow suite passed137/137 checks: virtual keyboard/mouse/gamepad menu navigation, all-profile paint and true roster capabilities, countdown/racing Quit Race, pause/settings, upright/flipped/slope/tree/moving-car recovery, finish-adjacent recovery rejection, no-support waiting, notification cooldown, physical mailbox destruction/debris cleanup/restoration, results runoff/parking and opponent cleanup. Record migration is covered separately by the111-rule suite. Physical controller and human UI/handling acceptance are not claimed.
...
Build identity: Windows x64 `0.6.1-review2`, executable source `566b4180fa633d182deb7559ae31c025f36ab52f`, Assembly-CSharp SHA256 `9C5258E8273EDE4BDBD8FE4CAE4FE8EC44CFFC42DDF353B366A8C7BADAC0FD84`. The source tree's Assets/Packages/ProjectSettings matched that commit after packaging. Unity build succeeded with zero errors and two warnings: future mesh-collision pre-bake compatibility, and intentionally absent optional Runtime Pipeline configuration.

The231-file package, extracted ZIP and complete `Builds/Latest` are SHA256-verified against one manifest. ZIP SHA256: `443232DC867FEF225CF0DFCD052A033707E6C8308F644BCF6E13D45550F1362A`. `Play-Racer.cmd` was launched on Dan's actual desktop; Computer Use verified the Latest executable and menu (`launcher-process.json`, `launcher-window.jpg`). Saved settings remained unchanged. Earlier restricted-desktop diagnostics are retained but are not presented as user-desktop verification. No upload or distribution was performed.
