...
**The public catalog has not been published yet.** Offline Play works; Check will report unavailable until an approved release is published. This is a Windows-tested native prototype for Deck/Proton review, not a claim of physical Deck acceptance.
...
Physical Deck, controller hardware, hosted update, and existing-user-prefix acceptance remain pending. Automated Windows fixtures are not substitutes for those checks.
