...
System.IO.File.WriteAllLines(dir+"/terrain-before.txt",rows.OrderBy(x=>x));
...
System.IO.File.WriteAllLines(dir+"/colliders-before.txt",colliders);
System.IO.File.WriteAllLines(dir+"/sites-before.txt",GameObject.Find(Racer.Editor.Phase6Review.Root).transform.Cast<Transform>().Select(t=>t.name+"|"+t.position.ToString("F6")+"|"+t.rotation.ToString("F6")).OrderBy(x=>x));
...
System.IO.File.WriteAllText(dir+"/inventory-before.json",Newtonsoft.Json.JsonConvert.SerializeObject(inventory,Newtonsoft.Json.Formatting.Indented));return inventory;
