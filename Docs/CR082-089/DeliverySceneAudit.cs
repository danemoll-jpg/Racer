...
foreach(var path in Racer.Editor.ReverseReviewRelease.Scenes)
{
 ...
 foreach(var b in UnityEngine.Object.FindObjectsByType<Racer.WoodlandRoute>(UnityEngine.FindObjectsSortMode.None))
 {
  ...
  if(race.reverseCourse)Racer.LivingWorldValidation.Capture("Docs/CR082-089/delivery-"+race.gameObject.scene.name+"-"+b.title.Replace(' ','-')+".png",road.At(b.entryRoad-24,out var f)+UnityEngine.Vector3.up*2.6f,road.At(b.entryRoad+15,out _)+UnityEngine.Vector3.up*1.5f);
 }
 ...
 System.IO.File.WriteAllLines("Docs/CR082-089/route-delivery-"+race.gameObject.scene.name+".txt",rows);
 if(race.reverseCourse)Racer.LivingWorldValidation.Capture("Docs/CR082-089/delivery-"+race.gameObject.scene.name+"-grid.png",spawn.position-spawn.forward*6+UnityEngine.Vector3.up*3,spawn.position+spawn.forward*25+UnityEngine.Vector3.up);
}
UnityEditor.SceneManagement.EditorSceneManager.OpenScene(initial);
return "Saved delivery route inventories and approach views; no scene edits";
