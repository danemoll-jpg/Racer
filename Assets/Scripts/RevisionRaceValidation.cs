...
namespace Racer
{
    public sealed class RevisionRaceValidation : MonoBehaviour
    {
        ...
        public static void Launch()=>new GameObject("Revision full mixed race").AddComponent<RevisionRaceValidation>();
        [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.AfterSceneLoad)]
        static void Boot()
        {
            var args=System.Environment.GetCommandLineArgs();
            if(System.Array.IndexOf(args,"-racerRevisionRace")<0 || System.Array.IndexOf(args,"-racerTestSave")<0) return;
        ...
        IEnumerator Start()
        {
...
#if UNITY_EDITOR
            race.Flow.UseValidationSave(Path.GetFullPath("Temp/revision-race-"+Difficulty));
#endif
            string root="Docs/CR028-033/"+(Application.isEditor?"editor":"standalone")+"-race-"+Difficulty;
            Directory.CreateDirectory(root);
            race.Flow.OpenGarage(); race.Flow.SelectVehicle("original"); race.Flow.CloseGarage();
            race.opponents=race.traffic=true; race.difficulty=Mathf.Clamp(Difficulty,0,2); race.laps=Laps;
            ...
            frames.Sort(); var rows=new List<string>{$"Difficulty={race.DifficultyName}; laps={Laps}; traffic=4; shared capabilities; ordinary-frame motor commands. Reference pilot is NOT a human-competition acceptance test.",$"Final={race.ClassificationFinal}; frames={frames.Count}; median={frames[frames.Count/2]:F2}ms; p95={frames[(int)(frames.Count*.95f)]:F2}ms"};
            for(int i=0;i<4;i++)
            {
