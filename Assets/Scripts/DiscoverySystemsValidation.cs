...
namespace Racer
{
    public sealed class DiscoverySystemsValidation:MonoBehaviour
    {
        ...
        IEnumerator Views()
        {
            ...
            var clue=FindObjectsByType<Transform>().First(t=>t.name.StartsWith("Acorn clue / "));var cp=clue.position;camera.transform.SetPositionAndRotation(cp+new Vector3(2,1.4f,-2),Quaternion.LookRotation(cp+Vector3.up*.35f-(cp+new Vector3(2,1.4f,-2))));yield return null;ThreeFeatureValidation.CaptureUi(dir+"/stone-clue.png");
