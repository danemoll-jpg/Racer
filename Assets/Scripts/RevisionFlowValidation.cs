...
namespace Racer
{
    public sealed class RevisionFlowValidation : MonoBehaviour
    {
        RaceFlow flow;
        RaceDirector race;
        Gamepad pad; Keyboard keys; Mouse mouse;
        readonly List<string> rows=new();
        public static void Launch()=>new GameObject("Revision flow validation").AddComponent<RevisionFlowValidation>();
        string Output=>"Docs/CR028-033/"+(Application.isEditor?"editor":"standalone")+"-flow.txt";
        [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.AfterSceneLoad)]
        static void Boot() { var args=System.Environment.GetCommandLineArgs(); if(System.Array.IndexOf(args,"-racerRevisionFlow")>=0 && System.Array.IndexOf(args,"-racerTestSave")>=0) { Application.runInBackground=true; Launch(); } }
        ...
        IEnumerator Start()
        {
            ...
            Application.runInBackground=true; Directory.CreateDirectory("Docs/CR028-033");
#if UNITY_EDITOR
            flow.UseValidationSave(Path.GetFullPath("Temp/revision-flow-"+System.Guid.NewGuid().ToString("N")));
            ...
            foreach(var profile in VehicleProfile.All)
            {
                ...
                Check(flow.SelectedColor==3,"Gamepad swatch "+profile.Id);
            ...
            Check(flow.SelectedColor==3,"Color survives profile switches and restarts");
            flow.StartRace(); while(flow.State!=RaceFlow.Stage.Racing) yield return null;
            ...
            ScreenCapture.CaptureScreenshot(Path.GetFullPath("Docs/CR028-033/hud-"+Screen.width+"x"+Screen.height+".png"));
            ...
            rows.Add("DONE"); File.WriteAllLines(Output,rows); Destroy(gameObject);
        }
