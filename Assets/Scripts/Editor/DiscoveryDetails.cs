...
namespace Racer.Editor
{
    public static partial class DiscoveryAuthoring
    {
        ...
        static void Map()
        {
            ...
            foreach(var route in new[]{street,owner.road}.Concat(collection.routes).Distinct()){route.Initialize();float length=route.Length;if(route.name=="Kyle descending driveway"||route.name=="House 3 valley driveway"||route.name=="Summit approach and safe return"){length=0;for(int i=1;i<route.points.Length;i++)length+=Vector3.Distance(route.points[i-1],route.points[i]);}for(float s=0;s<length;s+=2){var uv=ExplorationMap.Normalized(route.At(s,out _));int x=Mathf.FloorToInt(uv.x*w),y=Mathf.FloorToInt(uv.y*h);if(x>=0&&x<w&&y>=0&&y<h)colors[y*w+x]=route==street?new(.65f,.64f,.58f):new(.61f,.43f,.23f);}}
