...
namespace Racer.Editor
{
    public static class CR112Release
    {
        ...
        public static void Build(string tag)
        {
            ...
            File.WriteAllText("Docs/CR112-118/build-"+tag+".txt",report.summary.result+" errors="+report.summary.totalErrors+" warnings="+report.summary.totalWarnings);
            if(report.summary.result!=UnityEditor.Build.Reporting.BuildResult.Succeeded)throw new Exception("Build failed");
            File.WriteAllText(dir+"/VERSION.txt","Racer "+Version+" / Woodstock Rush\nCR-112–118\nSafety checkpoint 16ccff01e05c666d312c02df443dd4b22564f7a1\n");
