...
{
    public static class RevisionReleaseBuild
    {
        public const string Version="0.5.0-review1";
        public const string Output="Builds/Racer-"+Version+"-Windows";
        public static void Candidate(string output="Temp/RevisionPlayer")=>BuildTo(output,false);
        ...
        static void BuildTo(string output,bool release)
        {
            ...
            File.WriteAllText("Docs/CR028-033/"+(release?"release":"candidate")+"-build.txt",$"{report.summary.result}; errors={report.summary.totalErrors}; warnings={report.summary.totalWarnings}; bytes={report.summary.totalSize}; duration={report.summary.totalTime}\n"+string.Join("\n",report.steps.SelectMany(s=>s.messages).Where(m=>m.type==LogType.Error || m.type==LogType.Warning).Select(m=>m.content)));
