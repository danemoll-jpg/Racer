...
namespace Racer.Editor
{
    ...
    {
        public static float RefinedSummitHeight(float s){if(s<70)return 145+s*.08f;if(s<=100){float t=(s-70)/30;return 150.6f+2.4f*t+4.8f*t*t;}if(s<122)return Mathf.Lerp(157.8f,154.5f,Smooth(100,122,s));return 154.5f-(s-122)*.015f;}
        public static void RefineSummit()
        {
            foreach(var path in ReverseReviewRelease.Scenes){EditorSceneManager.OpenScene(path);var race=Object.FindAnyObjectByType<RaceDirector>();var collection=race.GetComponent<ExplorationCollection>();var root=GameObject.Find("CR094 summit launch").transform;
                ...
                Terrain("summit-landing-v2",p=>{if(p.x<760||p.x>1100||p.z<120||p.z>290)return p;float s=1070-p.x,side=Mathf.Abs(p.z-190);float width=s>110?20:12;float blend=Smooth(-25,0,s)*(1-Smooth(260,300,s))*(1-Smooth(width,width+18,side));
                ...
                ClearTrees(p=>Near(p,exit,out _)<9||p.x>775&&p.x<970&&Mathf.Abs(p.z-190)<32);
                File.WriteAllText(Evidence+"/summit-refined-"+race.gameObject.scene.name+".txt","First playable driving retained. Launch tangent is now continuous from .08 to .4, eliminating the 40m/s pre-lip hop; lip 157.8m. Landing plateau 154.5m, .015 downhill grade, 40m width. Return loops north at runway height via (805,151,250), crosses at (850,151.5,190), then joins the EXISTING ridge at its matching elevation (901.34,150.48,153.63), avoiding the motorcycle's steep descent. No handling/scoring threshold changes.");Save();
            }
        }
    }
}
