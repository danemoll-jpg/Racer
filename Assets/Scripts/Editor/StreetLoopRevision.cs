...
public static class StreetLoopRevision {
 ...
 public static void Rebuild(){
  ...
  // Shared authored slots prevent a deliberate fresh generation from restoring mirrored rows.
  // This legacy rebuild remains guarded above and must not be run on the accepted scene.
  foreach(int side in new[]{-1,1})foreach(var slot in side<0?CR019Commercial.South:CR019Commercial.North){var road=CR019Commercial.Road(slot.x);var tangent=(CR019Commercial.Road(slot.x+.2f)-CR019Commercial.Road(slot.x-.2f)).normalized;var p=road+Vector3.Cross(tangent,Vector3.up).normalized*side*slot.setback;House(CR019Commercial.Id(side,slot.source),p,false,20+(float)rng.NextDouble()*11,15+(float)rng.NextDouble()*5);}
  var woods=new GameObject("Woods replacing later subdivisions").transform;var reservedShortcut=Phase5Setup.Path();
  ...
  for(int i=0;i<7000;i++){var p=new Vector3(Mathf.Lerp(-740,735,(float)rng.NextDouble()),0,Mathf.Lerp(-680,525,(float)rng.NextDouble()));if(Near(p,out _)<ForestClearance)continue;bool clear=false;foreach(Transform b in buildings)if(Vector2.Distance(new(p.x,p.z),new(b.position.x,b.position.z))<22){clear=true;break;}
   ...
   if(clear || Phase6Vegetation.Reserved(p,route,reservedShortcut))continue;p.y=Surface(p);float h=9+(float)rng.NextDouble()*9;var key=new Vector2Int(Mathf.FloorToInt(p.x/160),Mathf.FloorToInt(p.z/160));if(!batches.TryGetValue(key,out var list))batches[key]=list=new();
