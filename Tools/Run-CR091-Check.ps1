param([string]$Runtime='Builds/Racer-0.16.0-review3-Windows/Racer.exe',[string]$Tag='final',[Parameter(Mandatory)][string]$Name,[Parameter(Mandatory)][string]$Flags,[int]$Limit=1800,[string]$ReuseSave="")
$ErrorActionPreference='Stop'
$project=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..'))
$exe=[IO.Path]::GetFullPath((Join-Path $project $Runtime))
$evidence=Join-Path $project "Docs/CR091-096/$Tag/$Name"
New-Item -ItemType Directory -Force -Path $evidence | Out-Null
$save=Join-Path $project ('Temp/cr091-'+[Guid]::NewGuid().ToString('N'))
...
$arguments="-batchmode -screen-width 1280 -screen-height 720 -racerTestSave `"$save`" -evidence `"$evidence`" -logFile `"$project/Logs/cr091-$Tag-$Name.log`" $Flags"
