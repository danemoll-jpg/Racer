...
onset=len(sys.argv)>2 and sys.argv[2]=='onset'
def read(path):
...
for species in ['Bird','Squirrel','Frog','Deer','Coyote','Bats']:
    if species=='Bats':
        if not (folder/'Bats-mix.wav').exists():continue
        clip='bat-flight';pitch=1
    else:
    ...
    begin,end=(.04,.45) if onset and species=='Deer' else (.25,1.9)
    source=source[int(begin*sr):int(end*sr)]
    ...
    rows.append(dict(species=species,clip=clip,pitch=pitch,sourceWindowSeconds=[begin,end],correlation=float(normalized[i]),matchedRms=mr,residualRms=rr,matchedToResidualDb=float(20*np.log10(max(mr,1e-12)/max(rr,1e-12))),peak=float(np.max(np.abs(mix))),matchAtSeconds=i/rate,interpretation='Waveform evidence at listener before OS endpoint; not human audibility or listening acceptance'))
(folder/('waveform-deer-onset-presence.json' if onset else 'waveform-presence.json')).write_text(json.dumps(rows,indent=2));print(json.dumps(rows,indent=2))
