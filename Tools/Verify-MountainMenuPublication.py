...
EVIDENCE=ROOT/'Docs/MountainMenu/hosted'
EVIDENCE.mkdir(exist_ok=True)
WORK=ROOT/'Temp'/('mountain-menu-hosted-'+str(time.time_ns()))
...
assert manifest['build']==24001 and manifest['version']=='0.24.1-review1'
...
assert state['game']['manifest']['build']==24001
...
result=dict(version=manifest['version'],build=24001,catalog=CATALOG,manifest=catalog['game'],publicDownload=True,pinnedSignatureVerified=True,allLatestFilesMatch=True,files=len(manifest['files']),startupReady=True,exit=code,work=str(WORK))
