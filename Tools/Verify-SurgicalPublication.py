...
EVIDENCE=ROOT/'Docs/SurgicalTracks/hosted'
EVIDENCE.mkdir(exist_ok=True)
WORK=ROOT/'Temp'/('surgical-tracks-hosted-'+str(time.time_ns()))
...
assert manifest['build']==26000 and manifest['version']=='0.26.0-review1'
...
assert state['game']['manifest']['build']==26000
...
result=dict(version=manifest['version'],build=26000,catalog=CATALOG,manifest=catalog['game'],publicDownload=True,pinnedSignatureVerified=True,allLatestFilesMatch=True,files=len(manifest['files']),startupReady=True,exit=code,work=str(WORK))
