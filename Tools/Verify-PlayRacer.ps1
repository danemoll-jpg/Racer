...
if(!$version.Contains('Racer 0.27.0-review1')){throw 'Latest version mismatch'}
...
$record=[ordered]@{entryPoint=(Join-Path $root 'Play-Racer.cmd');path=$launched.Path;version='0.27.0-review1';pid=$launched.Id;responsive=$launched.Responding;window=$launched.MainWindowTitle;verifiedAt=(Get-Date -Format o)}
$record | ConvertTo-Json | Set-Content (Join-Path $root 'Docs/SimpleLaurel/play-racer-launch.json')
