...
EVIDENCE=ROOT/'Docs/LocalRecovery/hosted'
EVIDENCE.mkdir(exist_ok=True)
WORK=ROOT/'Temp'/('local-recovery-hosted-'+str(time.time_ns()))
...
assert manifest['build']==25000 and manifest['version']=='0.25.0-review1'
...
assert state['game']['manifest']['build']==25000
...
result=dict(version=manifest['version'],build=25000,catalog=CATALOG,manifest=catalog['game'],publicDownload=True,pinnedSignatureVerified=True,allLatestFilesMatch=True,files=len(manifest['files']),startupReady=True,exit=code,work=str(WORK))
