...
EVIDENCE=ROOT/'Docs/SimpleLaurel/hosted'
EVIDENCE.mkdir(exist_ok=True)
WORK=ROOT/'Temp'/('simple-laurel-hosted-'+str(time.time_ns()))
...
assert manifest['build']==27000 and manifest['version']=='0.27.0-review1'
...
assert state['game']['manifest']['build']==27000
...
result=dict(version=manifest['version'],build=27000,catalog=CATALOG,manifest=catalog['game'],publicDownload=True,pinnedSignatureVerified=True,allLatestFilesMatch=True,files=len(manifest['files']),startupReady=True,exit=code,work=str(WORK))
