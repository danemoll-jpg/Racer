param([string]$Runtime='Builds/Racer-0.17.0-review1-Windows/Racer.exe',[string]$Tag='final',[Parameter(Mandatory)][string]$Name,[Parameter(Mandatory)][string]$Flags,[int]$Limit=1800,[string]$ReuseSave="",[int]$Width=1280,[int]$Height=720)
$ErrorActionPreference='Stop'
$project=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..'))
$exe=[IO.Path]::GetFullPath((Join-Path $project $Runtime))
$evidence=Join-Path $project "Docs/CR097-100/$Tag/$Name"
New-Item -ItemType Directory -Force -Path $evidence | Out-Null
$save=Join-Path $project ('Temp/cr097-'+[Guid]::NewGuid().ToString('N'))
...
$titleFlag=if($Flags -match '-titleCheck'){''}else{'-racerSkipTitle'}
$arguments="$titleFlag -batchmode -screen-width $Width -screen-height $Height -racerTestSave `"$save`" -evidence `"$evidence`" -logFile `"$project/Logs/cr097-$Tag-$Name.log`" $Flags"
