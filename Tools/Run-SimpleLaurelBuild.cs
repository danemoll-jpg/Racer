...
UnityEditor.EditorApplication.delayCall += () => {try { const string Version="0.27.0-review1";
  ...
  System.IO.File.WriteAllText("Docs/SimpleLaurel/build-release.txt",report);if(result.summary.result!=UnityEditor.Build.Reporting.BuildResult.Succeeded)throw new Exception(report);
  System.IO.File.WriteAllText(output+"/VERSION.txt",$"Racer {Version}\nOne straight Laurel ground ramp; recent shortcut strip removed; far side and recovery unchanged\nRollback checkpoint: e88f23e6\nUnity {Application.unityVersion}\n");
  ...
  System.IO.File.WriteAllText("Docs/SimpleLaurel/build-done.txt",report);

}catch(Exception e){System.IO.File.WriteAllText("Docs/SimpleLaurel/build-error.txt",e.ToString());}};return "Scheduled existing Windows release build";
