param([ValidatePattern('^[0-9]+\.[0-9]+\.[0-9]+(?:-[A-Za-z0-9-]+)?$')][string]$Version='0.10.0-review1')
...
if($Version -eq "0.10.0-review1"){Copy-Item -LiteralPath "$projectRoot/Docs/CR064-066/README-player.txt" -Destination "$stage/README.txt"}else{(Get-Content -LiteralPath "$projectRoot/Docs/CR061-062/README-player.txt" -Raw).Replace("0.9.1-review1",$Version) | Set-Content -LiteralPath "$stage/README.txt"}
...
if($Version -eq "0.10.0-review1"){Copy-Item -LiteralPath "$projectRoot/Docs/CR064-066/VEHICLE-ASSET-NOTICES.txt" -Destination $licenses -Force}else{Copy-Item -LiteralPath "$projectRoot/Docs/CR028-033/VEHICLE-ASSET-NOTICES.txt" -Destination $licenses -Force}
