param([ValidatePattern('^[0-9]+\.[0-9]+\.[0-9]+(?:-[A-Za-z0-9-]+)?$')][string]$Version='0.11.0-review1')
...
if($Version -in @("0.10.0-review1","0.11.0-review1")){Copy-Item -LiteralPath "$projectRoot/Docs/CR064-066/VEHICLE-ASSET-NOTICES.txt" -Destination $licenses -Force}else{Copy-Item -LiteralPath "$projectRoot/Docs/CR028-033/VEHICLE-ASSET-NOTICES.txt" -Destination $licenses -Force}
