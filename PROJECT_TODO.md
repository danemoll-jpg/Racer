...
**CURRENT SCOPE — CR-112–118 COURSE PREVIEW / Dan's reduced gate, 2026-09-21:** Stop extensive mountain/AI testing. The former four-lap/all-vehicle/center-and-offset/alternate/AI-event matrix is superseded. Existing route and first-playable flight evidence is sufficient for the review preview; no further track runs are planned. Deliver the implemented fixes with minimal compilation, tiny scoring fixtures, package/startup and the two authorized title-audio checks. Both mountain courses remain NOT ACCEPTED / awaiting Dan's layout and fun review. AI and incomplete vehicle coverage remain unverified. Ghosts are accepted and untouched; turkeys await Dan's sighting. After course approval, do not automatically start extensive testing: only targeted checks of new changes or issues Dan reports, especially AI. Launcher remains queued after this delivery checkpoint and must not delay this preview.
...
**Status:** IMPLEMENTED / targeted natural visit sequence passed; awaiting Dan review.
...
Status: playback-lifetime fix implemented; two final-player listener-output comparisons pass, human listening approval pending. The supplied MP3 and imported WAV decode identically through the complete 1.750208-second recording. Early title input previously stopped its coroutine and voice source; the persistent source now finishes across the title-to-menu transition, with radio held until the speech tail completes. Original voice, artwork and theme are unchanged. The available audio-input tool cannot audition sound; packaged PCM is identical and both listener-DSP captures retain the full ending (final 350-ms correlation 0.999975). Untouched extracted-ZIP startup retains the theme; early advance reaches the menu without interrupting the phrase. The untouched fixture checked its post-dismissal menu too early and retained a failed assertion; portable title startup passed. These captures cannot substitute for a claimed human/OS-endpoint listen.
