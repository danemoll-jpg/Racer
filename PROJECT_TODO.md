...
**Consistent testing entry point:** Dan launches `Play-Racer.cmd` at the project root, or `Builds/Latest/Racer.exe`. After each new validated playable build, update the complete `Builds/Latest` runtime folder (including VERSION.txt); never replace only its executable. Keep older versioned builds separately. Current Latest is 0.5.0-review1, source c57b6cce912b256de15da74a0e7ef19495bf8b48; combined revision awaiting Dan approval. A source commit alone does not update a compiled player.
