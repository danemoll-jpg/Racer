...
- [x] Original house markers reviewed; corrected memory retains only neighboring Houses 2 and 3. House 1 removed under CR-012; visual approval pending.
...
**Buildings/assets:** 25 retained houses and 22 businesses use 4 residential + 4 commercial prefabs under `Assets/Buildings/Phase6/`, shared roofs/materials, framed windows, entrances, modest roof/color variation and fictional storefront identities. All architectural detail is approximate, not historically verified. Walls/steps have box collision, pitched roofs have simple convex collision. Foundation bottoms extended 0.071m under House 2 and 0.359m under the residence at (-61.60,33.50,-511.50); tops and site transforms did not move.
**Generation:** `Phase6Buildings.Build()` applies this local batch and refuses a repeat. Older generators omit House 1 and reserve the yard from trees. `StreetLoopRevision.Rebuild()` now has an accepted-work guard; never run the old whole-environment rebuild over this scene. Guard inspected in source only. `RefreshVisualBatches()` refreshes only building meshes after deliberate building/prefab edits: 426 source renderers are disabled and 11 shared-material scene batches render their exact geometry; shop text and colliders remain independent. Do not re-enable source renderers alongside the batches.
**Preserved:** Car tuning (44m/s speed parameter, 13.5m/s² acceleration and all accepted handling/suspension), camera/input/reset, race/HUD/gates, loop, jump, shortcut, landing and road support. No runtime gameplay source or vehicle prefab changes. Retained scene changes are confined to building child lists/foundations, local forest parent, and the new visual-batch root. All surviving building site transforms match baseline.
