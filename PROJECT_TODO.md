...
**Consistent testing entry point:** Dan launches `Play-Racer.cmd` at the project root, or `Builds/Latest/Racer.exe`. After each new validated playable build, update the complete `Builds/Latest` runtime folder (including VERSION.txt); never replace only its executable. Keep older versioned builds separately. Current woodland review is 0.6.0-review1; Builds/Latest/VERSION.txt records the executable source. Refresh/verification is part of this delivery; combined revision awaits Dan approval. A source commit alone does not update a compiled player.
...
**Current delivery:** CR-034–039 implemented together as 0.6.0-review1, awaiting Dan's review. Black paint, original material smash sounds and remembered fences, stronger fair AI policies, four-lane northern Hwy 92 with faster local traffic, named breakable signs and three legal woodland stunt routes. Houses, yards and accepted shop ordering remain fixed. CR-013/018 stay deferred; CR-040 remains backlog and no second track/lake was built.
...
Scene: Assets/Scenes/StreetLoopGreybox.unity. Launcher: Play-Racer.cmd. Review build: Builds/Racer-0.6.0-review1-Windows; latest runtime and ZIP are refreshed during release verification. VERSION.txt identifies the actual executable source commit; package-verification.json records file hashes. Completion commit is the final documentation/evidence commit in Git history and the delivery response. Nothing is uploaded or distributed.
...
77/77 route/body/lane fixtures,137/137 visible standalone virtual-input flow checks,33/33 material/highway checks. All four vehicles have two successful clean approaches on each route; final motorcycle shoulder and recovery cases passed in the Editor and are rechecked in the committed-source player. Raw60-run comparison tables retain failures and identify superseding reruns. Creek saves roughly7–8s; Ridge4–6s; conservative Gully about0.4–1.0s, with faster riskier car passes around2s. Speeds/directions and every attempt: ROUTE-TIMINGS.md.
